...
exports.adminRescheduleStudent = exports.createCourseService = exports.createExamSlotWithBatches = void 0;
const config_1 = require("../config/config");
const createExamSlotWithBatches = async (courseId, startDate, endDate, physicalCapacity, batchDurationMinutes = 60, breakStartHour = 13, breakEndHour = 14, batchGapMinutes = 30) => {
    const client = await config_1.connection.connect();
    try {
        await client.query("BEGIN");
        // 1) Try to find the most recent slot linked to this course
        const existingSlotRes = await client.query(`SELECT es.id, es.startdate, es.enddate
       FROM course_slot cs
       ...
       LIMIT 1`, [courseId]);
        ...
        if (existingSlotRes.rowCount > 0) {
            // Found existing slot
            slotId = existingSlotRes.rows[0].id;
            // 2) Check if students are scheduled for this slot
            const scheduledCountRes = await client.query(`SELECT COUNT(*)::int AS total FROM student_schedule WHERE slot_id = $1`, [slotId]);
            ...
            if (scheduledCount > 0) {
                ...
                throw new Error(`Cannot update slot ${slotId}: ${scheduledCount} students are already scheduled. ` +
                    `Please reschedule students or use the admin reschedule flow before updating the slot.`);
            }
            // 3) Update exam_slot
            await client.query(`UPDATE exam_slot SET startdate = $1, enddate = $2, physical_capacity = $3
         WHERE id = $4`, [startDate, endDate, physicalCapacity, slotId]);
            ...
            const insertSlot = await client.query(`INSERT INTO exam_slot (startdate, enddate, physical_capacity)
         VALUES ($1, $2, $3)
         RETURNING id`, [startDate, endDate, physicalCapacity]);
            slotId = insertSlot.rows[0].id;
            // Link the slot to course
            await client.query(`INSERT INTO course_slot (course_id, slot_id) VALUES ($1, $2)`, [courseId, slotId]);
            action = "created";
        }
        ...
        while (batchStart < endDate) {
            ...
            if (batchStart.getHours() >= breakStartHour &&
        ...
        for (const b of batches) {
            await client.query(`INSERT INTO exam_batches (slot_id, batch_number, start_time, end_time, capacity)
         VALUES ($1, $2, $3, $4, $5)`, [slotId, b.number, b.start, b.end, physicalCapacity]);
        }
...
exports.createExamSlotWithBatches = createExamSlotWithBatches;
const createCourseService = async (code, title, level) => {
    const query = `
...
`;
    ...
    const { rows } = await config_1.connection.query(query, values);
    return rows[0];
};
...
const adminRescheduleStudent = async (studentId, courseId, newBatchId) => {
    // Step 1: Derive the slot from batch
    const batchRes = await config_1.connection.query(`SELECT slot_id FROM exam_batches WHERE id = $1`, [newBatchId]);
    ...
    const existingScheduleRes = await config_1.connection.query(`SELECT id FROM student_schedule 
     WHERE student_id = $1 AND course_id = $2`, [studentId, courseId]);
    let result;
    if (existingScheduleRes.rowCount > 0) {
        // Reschedule (update existing)
        const updateRes = await config_1.connection.query(`UPDATE student_schedule
       SET slot_id = $1, batch_id = $2, rescheduled = true
       WHERE student_id = $3 AND course_id = $4
       RETURNING *;`, [newSlotId, newBatchId, studentId, courseId]);
        result = { action: "rescheduled", schedule: updateRes.rows[0] };
    }
    else {
        // First-time schedule (insert)
        const insertRes = await config_1.connection.query(`INSERT INTO student_schedule (student_id, course_id, slot_id, batch_id, rescheduled)
       VALUES ($1, $2, $3, $4, false)
       RETURNING *;`, [studentId, courseId, newSlotId, newBatchId]);
        result = { action: "scheduled", schedule: insertRes.rows[0] };
    }
    return result;
};
exports.adminRescheduleStudent = adminRescheduleStudent;
